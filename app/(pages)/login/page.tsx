...
import { buttonStyling, inputStyling } from "@/app/_components/AddSet";
...
export default function LoginPage() {
    ...
    useEffect(() => {
        setTimeout(() => {
            setIsLoading(false);
            if(isLoading) {window.alert("Login Successful!");} // mimics API delay
        }, 3000)
    }, [isLoading]);
    ...
    return(
        <div className="bg-gray-300 p-10 flex flex-col justify-center items-center select-none">
            <div className="w-8/10 bg-gray-200 shadow-lg shadow-gray-400 rounded-lg">
                <h2 className={`${pixelifySans.className} text-center text-5xl m-5 font-bold`}>Login</h2>
                ...
                <form onSubmit={handleSubmit} className="grid grid-cols-2 p-5 gap-2">
                    ...
                    <button className={`${buttonStyling} bg-yellow-200 border-yellow-500 hover:bg-yellow-300`} type="button" onClick={handleRegister}>Register</button>
                    <button className={`${buttonStyling}`} type="submit">Login</button>
                </form>
            </div>
        </div>
    );
